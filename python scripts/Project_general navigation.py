...
input_placename = arcpy.GetParameter(1)
input_type = arcpy.GetParameter(2)
...
def location_distance_calc(fc,a_name,target_name):
...
def navigation(path,region,street,type,l_name):
	...
	arcpy.AddMessage ("Your destination is: {0}".format(l_name))
...
#input_placename = "Stanley Park"

filename,attribute_name = destination_info(destination_type)
p_x,p_y = getposition_citizen(citizenID)

l_x,l_y = location_distance_calc(filename,attribute_name,input_placename)
...
navigation(path,region,street,destination_type,input_placename)
